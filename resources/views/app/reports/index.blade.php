...
@section('content')
    <div class="page-stack">
        <section class="panel">
            <h3 class="section-title">Monthly Dues</h3>
            <p class="muted" style="margin-top:-4px">Open the monthly dues dashboard for paid/partial/unpaid tracking.</p>
            <a href="{{ route('reports.monthly-dues.index') }}" class="primary-btn"
                style="margin-top:10px;display:inline-flex">Open Monthly Dues Dashboard</a>
        </section>
        ...
        <section class="panel">
            ...
            <form method="POST" action="{{ route('reports.exports.transactions') }}" class="grid grid-4"
        ...
        <section class="panel">
            ...
            <form method="GET" action="{{ route('reports.investments.csv') }}" class="grid auto-fit"
            ...
            <form method="POST" action="{{ route('reports.exports.investments') }}" class="grid auto-fit"
                style="max-width:560px;margin-top:10px">
                @csrf
                <select name="status" class="select">
                    <option value="">All statuses</option>
                    <option value="active">Active</option>
        ...
        <section class="panel">
            ...
            <form method="GET" action="{{ route('reports.wallet.passbook.pdf') }}" class="grid auto-fit"
                style="max-width:560px">
            ...
            <form method="POST" action="{{ route('reports.exports.wallet-passbook') }}" class="grid auto-fit"
                style="max-width:560px;margin-top:10px">
                @csrf
                <select name="user_id" class="select" required>
                    <option value="">Select member</option>
                    ...
                    <tbody>
                        @forelse ($exports as $export)
                            <tr>
                                <td>{{ $export->type }}</td>
                                <td><span
                                        class="pill {{ $export->status === 'completed' ? 'pill-green' : ($export->status === 'failed' ? 'pill-red' : 'pill-blue') }}">{{ ucfirst($export->status) }}</span>
                                </td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="empty">No export history yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
